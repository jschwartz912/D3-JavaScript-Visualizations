//Jules Schwartz
//D3 Homework 2 - modified d3-filtering-and-animation-titus
//Date: 02/20/2016
...
function start() {
    ...
    d3.select(graph)
        ...
        .text('Filter Data')
        .on('click', function() {
            //Filter bars that should not be included based on
            //user input for gpa and department
            //*Refills un-needed bars WHITE*
            bars.selectAll('.bar')
                .filter(function(d) {
                    ...
                    if (dept == "ALL") {
                        ...
                        return (d.Department != dept) || (d.GPA < gpa);
                    }
                })
                .transition()
                .duration(function(d) {
            ...
            //Filter bars that should be included based on
            //user input for gpa and department
            //*Refills needed bars STEELBLUE*
            bars.selectAll('.bar')
                .filter(function(d) {
                    var e = document.getElementById("DEPT");
                    var gpa =document.getElementById("gpa").value;
    ...
    // D3 will grab all the data from "Courses.csv" and make it available
    ...
    d3.csv('Courses.csv', function(d) {
        ...
        yScale.domain(data.map(function(d) {
            return d.Department + " " + d.CourseNum;
        }));
        ...
        bars.append('g')
            ...
            .attr('x', 62)
            .attr('y', function(d) {
                return yScale(d.Department + " " + d.CourseNum);
            })
            .attr('width', function(d) {
